...
// ── Share helper ───────────────────────────────────────────────────────────
async function shareTodayResults(games, players, dateStr, setCopied, zingerStyle, zingerOnly, setSharedText, setSharedStyleName) {
  const puzzleNum = getTodaysPuzzleNum(games, dateStr);
  const dayGames = puzzleNum
    ? games.filter(g => (g.puzzleNum && parseInt(g.puzzleNum) === puzzleNum) || (!g.puzzleNum && g.date === dateStr))
    : games.filter(g => g.date === dateStr);
  ...
  try {
    ...
    const absentLine = absentPlayers.length > 0 ? `\nDid not submit today: ${absentPlayers.join(", ")}` : "";
    const zingerPrompt = `${stylePrompt} for a friend group's daily Connections Sports Edition competition. Write exactly 1-2 sentences commenting on today's results entirely in the voice and style described above — do not break character or insert generic humor that doesn't fit the persona. IMPORTANT: the results below are listed in finishing order — 1st place won, last place lost. Be accurate about who beat whom. Keep it under 40 words.

Today's final standings (1st = best, lower time = better):
${entries.map((e, i) => {
  const medals = ["🥇","🥈","🥉"];
  const place = i === 0 ? "1st place WINNER" : i === entries.length-1 ? `${i+1}th place LAST` : `${i+1}${i===1?"nd":"rd"} place`;
  return `${medals[i] || (i+1)+"."} ${place}: ${e.name} — ${e.dnf ? "DNF" : e.finalTime}${e.adjustments?.length ? " ("+e.adjustments.map(a=>a.label).join(", ")+")" : ""}`;
}).join("\n")}${absentLine}`;
...
const DEFAULT_ZINGER_STYLES = [
  ...
  { id: "attenborough", label: "David Attenborough", active: true, prompt: "You are Sir David Attenborough, the legendary BBC nature documentary narrator. Treat the puzzle competition as if observing a rare species in the wild. Speak in hushed, reverent tones about the contestants as creatures navigating their natural habitat. Use phrases like 'Here we observe...', 'Remarkable', 'In all my years...', and dramatic pauses before revealing outcomes. Draw parallels between human puzzle-solving behavior and the natural world. Build slowly to a majestic conclusion." },
  { id: "rueben",   label: "Rueben McDaniel", active: true,  prompt: "You are Rueben McDaniel, beloved and legendary MBA professor of Complexity Science. You are brilliant, irreverent, deeply wise, and utterly unfiltered. Your style: raw Southern wit, mild profanity used naturally ('what the shit', 'son of a bitch', 'don't give a shit'), occasional Complexity Science references (edge of chaos, emergence, nonlinear systems), and sharp observations about human nature. You are tenured, old, and do not sugarcoat anything. When someone wins impressively, you might say 'praise the Lord and find out how it may hurt you.' When someone struggles, you call it like you see it. You don't mind people screwing up, just do it with class. You might reference Mann Gulch — 'Mann Gulch was a pretty complex situation, you make a few mistakes and you might not survive' — as a metaphor for consequential failure." },
];
...
export default function App() {
  ...
  const [sharedStyleName, setSharedStyleName] = useState(null); // style used for last share
    ...
    <Screen title="🏆 MBA Friends Connections" T={T}>
      <div style={{ marginTop:20 }}>
        ...
        <div style={{ display:"flex", gap:6, marginBottom:10 }}>
          <Btn T={T} variant="ghost" onClick={async () => { if (sharing) return; setSharing(true); setShowStylePicker(false);
              const styleToUse = selectedStyle || pickRandomZingerStyle(zingerStyles);
              await shareTodayResults(data.games, names, todayStr(), setCopied, styleToUse, zingerOnly, setSharedText, setSharedStyleName);
              setSharing(false); }} style={{ flex:1, padding:12, boxSizing:"border-box" }}>{copied ? "✓ Copied!" : sharing ? "✍️ Generating…" : "📤 Share Today's Results"}</Btn>
          {canEditZingerStyles(activePlayer) && (
        ...
        {sharedText && (
          <Card T={T} style={{ marginBottom:14 }}>
            <div style={{ display:"flex", justifyContent:"space-between", alignItems:"center", marginBottom:10 }}>
              <div style={{ fontSize:11, color:T.muted, letterSpacing:"0.07em", textTransform:"uppercase" }}>{sharedStyleName ? `${sharedStyleName}'s Take` : "Today's Share Text"}</div>
